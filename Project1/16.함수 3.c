...
//6. va_args(args, 자료형)은 args로부터 하나의 값을 가져오고 그 값을 자료형으로 명시합니다.
//7. va_end(args) : 가변인자에 대한 처리를 종료하고 기능을 제거합니다. (제거하지 않으면 프로그램에 데이터 남음 필수로 제거)
...
//2.배열을 받는 경우
//배열을 전달할 경우에는 c언어가 자동적으로 배열의 길이를 알
