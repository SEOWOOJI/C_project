...
void Backtracking(int N, int M, int depth, int path[])
{
	...
	//5. 일반적인 경우 진행할 작업
	//현재 문제에는 1부터 N까지의 범위를 사용하고 있습니다.
	for (int i = 1; i <= N; i++)
	{
		//used[i]가 사용되지 않은 상태라면
		if (!used[i])
		{
			used[i] = 1;//사용 중이라고 설정하겠습니다. (c프로그래밍에서 1은 true, 0은 false를 의미합니다.)
			path[depth] = i; //현재의 i 값을 작업 순서(depth)에 있는 path에 등록합니다.

			//7. 백트래킹 작업(다음 단계로 넘어가기)
			//depth(진행순서)의 값을 1증가시켜서, 다시 함수(Backtracking)을 호출하는 작업을 진행합니다.
...
{
	int N, M;
	scanf("%d %d", &N, &M); //N과 M을 순서대로 입력(띄어쓰기나 Enter기준)

	Backtracking(N, M, 0, path);
	//배열의 시작값이 0이기 때문에 depth는 0을 넣어서 순차적으로 작업이 진행될 것입니다.

	//해당 문제의 작업 순서
	//1. scanf에 의해 N,M이 결정됩니다.
	//2. Backtracking(3,2,0,path) 호출이ㅣ 진행됩니다.
	//3. 내부에서 M과 depth(0)은 서로 같지 않음. 반복문 실행
	//   i = 0, used[1] = 1, path[0] = 1
