...
int main()
{
	...
	job = "Programmer"; //가리키는 값이 변경된 것이므로 수정 가능
	printf("%s, %s", name, job);
	...
	//3. char* 형태
	char* shop[] = { "딸기 바나나","딸기 사과" };

	//함수 1.strlen()
	char array1[] = "C Programming";
	char array2[] = "씨 프로그래밍";

	printf("array1의 문자열의 길이 = %1u\n", strlen(array1));
	//% : 서식 지정자
	//1 : long(정수)
	...
	char smaple4[20];
	strncpy(sample4, smaple3, 5); //비어있는 값에다 복사해줄 경우 이상한 값이 들어가게 됨
	...
	char sample5[] = "C언어";
	char sample6[] = "프로그래밍";
	strcat(sample5, sample6); //합칠 경우 문자열1 쪽이 길이가 더 커야 오류가 없음
	...
	//e.g.) "ApplePIE"
	//e.g.) "ApplfPIE"
	//이 둘을 strcmp로 비교할 경우 나올 갑은?

	//이 함수의 용도 : strcmp로 비교한 결과 값이0이면 같다는 점을 이용해서 문자열에 대한 확인 용도
