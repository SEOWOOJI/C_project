...
//포인터의 용도
//어떤 변수의 주소를 가리키는 역할
//새로만드는게 아닌, 기존의 값을 활용해 가리키고 있는 곳의 변수를
//수정하는 것이 가능합니다.
...
//e.g.) 자동차, 냉장고, 스마트 워치 등의 기능 담당 하드웨어를 제어하는 
//소프트웨어에 대한 제작
...
int main()
{
	int value = 100;
	int *ptr_value = &value;
	//1. *(asterisk) : 숫자 *숫자로 쓰이는 경우에는 곱하기 기호로 사용
	//                 자료형으로 쓰이는 경우에는 주소를 값으로 바꿔주는 기능
	//                 포이터에 대한 표현으로 사용합니다.

	//2. &(ampersand) : 값 &값으로 쓰일 경우에는 해당값에 대한 비트 AND 연산을 진행하는 기호
	//                  조건식 && 조건식으로 쓰일 경우는 조건식 AND 연산을 진행하는 기호
	//                  &변수명으로 쓰일 경우에는 해당 값을 주소로 바꿔주는 기능(변수 -> 주소)

	//3. %p : 포인터의 주소를 출력하는데 사용됩니다.

	printf("value의 값 : %d\n", value); //변수의 값은 그대로 사용
	printf("value의 값 : %p\n", &value); //변수의 주소 표현 시 & 사용
	printf("ptr_value의 값 : %d\n", *ptr_value); //포인터의 변수의 값 표현을 위해 * 사용
	printf("ptr_value의 값 : %p\n", ptr_value); //포인터는 주소기이기 때문에 그대로 표현
	...
	*ptr_value = 7;
	...
	// *의 개수가 늘어날 때마다 3중, 4중 포인터..로 늘어나게 됩니다.(n중 포인터)
	...
	//현재 데이터의 관계
	//ppptr_value -----> pptr_value -----> ptr_value ------> value -----> 7
